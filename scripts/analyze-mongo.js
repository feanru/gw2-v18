...
async function analyzeCollection(db, name) {
  ...
  for (const query of queries) {
    try {
      ...
      const summary = winningPlan?.planSummary
        || winningPlan?.stage
  ...
  for (const entry of report) {
    ...
    if (Array.isArray(entry.explains)) {
      for (const explain of entry.explains) {
        ...
        const plan = explain.planSummary
          || explain.winningPlan?.planSummary
          || explain.winningPlan?.stage
          || explain.winningPlan?.inputStage?.stage
          || 'unknown';
